...
int valSelect = 0; // Valeur sélecteur
...
void rotationGestionary(char rotDir){
  if(select){
    ...
    if(rotDir == 'H'){
      valSelect = (valSelect == maxSelect) ? 0 : valSelect+1;
    }else if(rotDir == 'T'){
      valSelect = (valSelect == 0) ? maxSelect : valSelect-1;
    }
  }
}
// ----------------
...
void loop(){
  bool pushed = !digitalRead(encoderSwitchPin);
